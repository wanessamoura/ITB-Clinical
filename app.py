...
class NumberedCanvas(canvas.Canvas):
    """Canvas que exibe paginação no formato 'Página X de Y'."""
    ...
    def save(self):
        ...
        for state in self._saved_page_states:
            ...
            self._draw_total_page_number(total_pages)
            canvas.Canvas.showPage(self)
        canvas.Canvas.save(self)

    def _draw_total_page_number(self, total_pages):
        self.saveState()
        self.setFont("Helvetica", 7.5)
        self.setFillColor(colors.grey)
        self.drawCentredString(A4[0] / 2, 0.75 * cm, f"Página {self._pageNumber} de {total_pages}")
        self.restoreState()
...
def pdf_report(d):
    ...
    doc.build(story, onFirstPage=add_footer, onLaterPages=add_footer, canvasmaker=NumberedCanvas)
    b.seek(0)
    return b.getvalue()
